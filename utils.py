...
def get_sinusoid_encoding_table(d_model, n_position=366, config=None):
    """
    参数:
    ...
    """
    if len(config['static_list']) > 0:
        d_model = d_model - len(config['static_list'])

    def get_position_angle_vec(position):
        return [position / np.power(10, 2 * hid_j / 256) for hid_j in range(d_model)]
    ...
    sinusoid_table = np.concatenate([sinusoid_table, sinusoid_table], axis=1)
...
def _rv(y_true, y_pred):
    ...
    # 计算相对变异性
    rv = np.sqrt(
        np.mean((y_pred - mean_pred) ** 2) / np.mean((y_true - mean_true) ** 2)
    )
...
def _plotloss(cfg, epoch_losses):
    ...
    plt.switch_backend(
        "Agg"
    )  # 后端设置'Agg' 参考：https://cloud.tencent.com/developer/article/1559466

    plt.figure(figsize=(12.8, 7.2))
    for i in range(cfg["num_repeat"]):
        plt.plot(epoch_losses[i], label=cfg["label"][i])  
        # epoch_losses 传入模型训练中的 loss[]列表,在训练过程中，先创建loss列表，将每一个epoch的loss 加进这个列表
